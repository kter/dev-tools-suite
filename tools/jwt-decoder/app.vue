  ...
  <div class="min-h-screen bg-gray-50">
    <div class="container mx-auto px-4 py-8">
      <header class="text-center mb-8">
        <h1 class="text-4xl font-bold text-gray-900 mb-2">JWT Decoder</h1>
        <p class="text-gray-600">Decode and validate JSON Web Tokens with detailed information display</p>
      </header>
      ...
      <div class="max-w-6xl mx-auto">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
          <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-semibold text-gray-900">JWT Token</h2>
          ...
          <textarea
            ...
            class="w-full h-32 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-sm resize-none"
          ></textarea>
          
          <!-- Token validation status -->
          <div v-if="tokenError" class="mt-2 text-red-600 text-sm">
          ...
          <div v-else-if="jwtToken && decodedToken" class="mt-2 text-green-600 text-sm">
          ...
          <div v-if="jwtToken && !tokenError" class="mt-4">
            <div class="text-sm text-gray-600 mb-2">Token Structure:</div>
            <div class="font-mono text-xs break-all">
              <span class="bg-red-100 text-red-800 px-1">{{ tokenParts.header }}</span>
              <span class="text-gray-400">.</span>
              <span class="bg-purple-100 text-purple-800 px-1">{{ tokenParts.payload }}</span>
              <span class="text-gray-400">.</span>
              <span class="bg-blue-100 text-blue-800 px-1">{{ tokenParts.signature }}</span>
            </div>
            <div class="flex gap-4 mt-2 text-xs">
              <span class="text-red-800">Header</span>
              <span class="text-purple-800">Payload</span>
              <span class="text-blue-800">Signature</span>
            </div>
          </div>
        </div>
        ...
        <div v-if="decodedToken" class="space-y-6">
          ...
          <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex justify-between items-center mb-4">
              ...
              <button
                ...
                class="px-3 py-1 text-sm bg-red-100 text-red-700 rounded hover:bg-red-200 transition-colors flex items-center gap-1"
              >
            ...
            <pre class="bg-gray-50 p-4 rounded border text-sm overflow-x-auto"><code>{{ JSON.stringify(decodedToken.header, null, 2) }}</code></pre>
            
            <!-- Header fields explanation -->
            <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div v-if="decodedToken.header.alg">
                <div class="text-sm font-medium text-gray-700">Algorithm</div>
                <div class="text-sm text-gray-600">{{ decodedToken.header.alg }}</div>
                <div class="text-xs text-gray-500">{{ getAlgorithmDescription(decodedToken.header.alg) }}</div>
              </div>
              <div v-if="decodedToken.header.typ">
                <div class="text-sm font-medium text-gray-700">Type</div>
                <div class="text-sm text-gray-600">{{ decodedToken.header.typ }}</div>
              </div>
              <div v-if="decodedToken.header.kid">
                <div class="text-sm font-medium text-gray-700">Key ID</div>
                <div class="text-sm text-gray-600">{{ decodedToken.header.kid }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex justify-between items-center mb-4">
              ...
              <button
                ...
                class="px-3 py-1 text-sm bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors flex items-center gap-1"
              >
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path>
                </svg>
            ...
            <pre class="bg-gray-50 p-4 rounded border text-sm overflow-x-auto"><code>{{ JSON.stringify(decodedToken.payload, null, 2) }}</code></pre>
            
            <!-- Payload claims -->
            <div class="mt-4">
              <h4 class="text-md font-medium text-gray-800 mb-3">Standard Claims</h4>
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div v-for="claim in standardClaims" :key="claim.key" v-show="decodedToken.payload[claim.key]">
                  <div class="text-sm font-medium text-gray-700">{{ claim.name }}</div>
                  <div class="text-sm text-gray-600">{{ formatClaimValue(claim.key, decodedToken.payload[claim.key]) }}</div>
          ...
          <!-- Token Status -->
          <div class="bg-white rounded-lg shadow-md p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">Token Status</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <!-- Issued At -->
              <div v-if="decodedToken.payload.iat" class="p-4 bg-blue-50 rounded-lg">
                <div class="flex items-center gap-2 mb-2">
                  <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                  </svg>
                  <div class="font-medium text-blue-800">Issued At</div>
                </div>
                <div class="text-sm text-blue-700">{{ formatTimestamp(decodedToken.payload.iat) }}</div>
              </div>
              ...
              <div class="p-4 bg-gray-50 rounded-lg">
                <div class="flex items-center gap-2 mb-2">
                  ...
                  <div class="font-medium text-gray-800">Algorithm</div>
                </div>
                <div class="text-sm text-gray-700">{{ decodedToken.header.alg || 'None' }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="bg-gray-50 p-4 rounded border font-mono text-sm break-all">
        ...
        <!-- Information -->
        <div v-if="!decodedToken" class="bg-blue-50 border border-blue-200 rounded-lg p-6">
