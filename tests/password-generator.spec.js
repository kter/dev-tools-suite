...
test.describe('Password Generator Ko-fi Widget', () => {
  test('should display Ko-fi widget on password generator page', async ({ page }) => {
  ...
  test('should not interfere with password generation', async ({ page }) => {
    ...
    // Check that password generation works with widget present
    await expect(page.locator('h1')).toContainText('Password Generator');

    // Test that generate button is still accessible
    const generateButton = page.locator('button:has-text("Generate Password")');
